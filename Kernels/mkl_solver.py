...
def primal_dual_opt(X, y, m, kernel_type, order, gap=10e-4, inner_tol=10e-1, weight_threshold=0.01, maxouter_iter=100, maxinner_iter=10, verbose=True):
    ...
    while duality_gap > gap and gap > 0:
        ...
        gradient_j = [compute_gradient(i, X, y_outer, alpha)
        ...
        while J_hat < J_d:
            ...
            J_hat = compute_dual(X, y, kernel_list, d_hat,
                                 C, compute_gap=False)
            if verbose:
        ...
        gamma_step = line_search(X, y, kernel_list, D,
                                 d_m, gamma_max, disc=line_search_steps)
